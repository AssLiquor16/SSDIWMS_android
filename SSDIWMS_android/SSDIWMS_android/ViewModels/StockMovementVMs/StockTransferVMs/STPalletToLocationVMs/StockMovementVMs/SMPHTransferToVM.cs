...
using SSDIWMS_android.ViewModels.StockMovementVMs.StockTransferVMs.STPalletToLocationVMs.StockMovementVMs.SMPHTransferToPopSubVMs;
using SSDIWMS_android.Views.StockMovementPages.StockTransferPages.STPalletToLocationPages.StockMovementPages.SMPHTransferToPopSubPages;
using System;
...
namespace SSDIWMS_android.ViewModels.StockMovementVMs.StockTransferVMs.STPalletToLocationVMs.StockMovementVMs
{
    ...
    public class SMPHTransferToVM : ViewModelBase
    {
        ...
        public SMPHTransferToVM()
        {
            ...
            MessagingCenter.Subscribe<SMWhLocListPupVM, WarehouseLocationModel>(this, "SetNewWarehouseLocation", async (sender, e) =>
            {
                NewWhLoc = e;
                                    ...
                                    foreach (var serverPalletd in PalletDetailsList)
                                    {
                                        ...
                                        if (newstckcrditms.Count() == 0)
                                        {
                                            ...
                                            var nonexiststckcrd = new StockCardsModel
                                            {
                                                ...
                                                OnCommited = 0,
                                                OnBegginning = 0,
                                                ItemCategory1 = imstr.Category ??= "Undefined",
                                                ItemCategory2 = imstr.Division ??= "Undefined",
                                                Brand = imstr.Brand ??= "Undefined",
                                                Warehouse_Location = newwhloc,
        ...
        {
            await PopupNavigation.Instance.PushAsync(new SMWhLocListPupPage());
        }
        private async Task PageRefresh()
        {
            if (Preferences.Get("SMPHTransferToInitialRefresh", false) == false)
            {
                ...
                await dependencies.notifService.LoadingProcess("End");
                Preferences.Set("SMPHTransferToInitialRefresh", true);
            }
